...
# ...
````
#### Dataset count using [organization_list](https://docs.ckan.org/en/2.9/api/#ckan.logic.action.get.organization_list)
````python
api = 'https://geoscience.data.qld.gov.au/api/action/'
# make a get request using a dictionary object
response = requests.get(api + 'organization_list',
                        params=dict(
                            all_fields=True,
...
json_response = response.json()
print(json_response['result'][0]['package_count'])
# 131150

# Note: using params gets the same result as using this URL string
requests.get('https://geoscience.data.qld.gov.au/api/action/organization_list?all_fields=True&include_dataset_count=True') 
````
